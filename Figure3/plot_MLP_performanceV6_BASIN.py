...
# === Load Data ===
X = np.load('LHS_param_values_spatialBASINs_50000.npy')           # Shape: (n_samples, 195)
Y = np.load('LHS_Y_flow_50000.npy')                               # Shape: (n_samples, 180)
...
class SubbasinMLP(nn.Module):
    def __init__(self, input_dim=195, output_dim=180):
        super(SubbasinMLP, self).__init__()
...
# === Load trained model ===
model = SubbasinMLP().to(device)
model.load_state_dict(torch.load("mlp_subbasin_model_best.pth", map_location=device))
model.eval()
loss_fn = RMSELoss()
...
ax4.text(0.05, 0.93, "(a4)",
        transform=ax4.transAxes, ha='left', va='top',
        fontsize=11, fontweight='bold',
        bbox=dict(facecolor='white', edgecolor='none', alpha=0.7, boxstyle='round,pad=0.2'))


fig.suptitle('(a) Subbasin-scale MLP surrogate',
...
plt.savefig('basin_scale_MLP_performance.png', dpi=300)
plt.show()
