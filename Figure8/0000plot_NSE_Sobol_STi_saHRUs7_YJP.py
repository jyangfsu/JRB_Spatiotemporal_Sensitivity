...
zhjs_shapefile_path = r'D:\Jinghe_SWAT\code_sptp_SA\yjps.shp'
gdf_zhjs= gpd.read_file(zhjs_shapefile_path)
...
ST_sb_tmp = np.load(r'Sobol_ST_NSE_spatialHRUs_streamed_design.npy')[:int(n_sb_params*n_subbasins)]
...
ST_hru_tmp = np.load(r'Sobol_ST_NSE_spatialHRUs_streamed_design.npy')[n_sb_params*n_subbasins:]
...
for i, col in enumerate([p + '_ST' for p in param_names]):
    ...
    if i == 0:
        ...
        gdf_hru.plot(column=col, cmap=cmap, vmin=0, vmax=0.30, legend=False, ax=ax)
        gdf_watersheld.plot(edgecolor='grey', facecolor='none', linewidth=0.2, legend=False, ax=ax)
        gdf_basin.plot(edgecolor='k', facecolor='none', linewidth=0.5, legend=False, ax=ax)
...
# =========================
fig.suptitle(
    '(b) HRU-scale $S_{Ti}$ based on NSE evaluated at YJP station',
