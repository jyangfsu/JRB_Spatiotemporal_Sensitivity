...
# Define the HRUs for each subbasin based on your example
hru_ranges = {
    1: range(1, 18),
    2: range(18, 42),
    3: range(42, 65),
...
param_hru_names = ['RCHRG_DP', 'CANMX', 'ESCO', 'CN2', 'SOL_AWC', 'SOL_BD', 'GWQMN', 'GW_DELAY']
...
gs = gridspec.GridSpec(2, 9, height_ratios=[1, 0.6], 
...
for i, p in enumerate(param_names):
    ...
    if i==0:
        ...
        for ii, (val, sub) in enumerate(zip(top10[col].values, top10['Subbasin'].values)):
            ax.text(val + 0.01, ii, f"Sub. {sub}", va='center')
